# UI Foundry Tables Studio

Standalone visual Tables generator built with Next.js, React, and TypeScript.

Standalone Tables component studio for building native, preview-honest, React-exportable UI patterns that match the UI Foundry product line.
...
- designers building Tables UI quickly
- developers who need editable React starting points
- marketplace buyers who want a self-contained Tables generator
...
The editor is focused on controls that are native to Tables and useful in real product work:
- columns, rows, caption, header, footer, zebra, density, selected rows, and empty state
- border, divider, sticky header intent, hover, focus, sortable-style header preview, and responsive overflow
- data, admin, pricing, comparison, and report table archetypes
- caption, scope, and semantic table accessibility
- React export for table structure

The goal is maximum useful depth without adding unrelated controls that would make the component stop reading as Tables.
...
This repo is intended to publish as Abid-Al-Hossain/UI_tables.
